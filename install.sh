...
die() {
	printf "${_CLR_ERR}[error]${_CLR_OFF} %s\n" "$*" >&2
	exit 1
}
# }}
...
step_codespaces() {
	is_codespaces || {
		log_info "Not Codespaces — skipping."
		return 0
	}
...
step_debian() {
	is_debian_like || {
		log_info "Not Debian-like — skipping."
		return 0
	}
	is_codespaces && {
		log_info "Codespaces — skipping debian step (see step_codespaces)."
		return 0
	}
...
step_linux() {
	is_linux || {
		log_info "Not Linux — skipping."
		return 0
	}
	is_codespaces && {
		log_info "Codespaces — skipping linux step."
		return 0
	}
...
step_macos() {
	...
	log_info "Setting up macOS xcode tools..."
	xcode-select --install

	# ── Homebrew ────────────────────────────────────────────────────────────
	if ! [[ -e /opt/homebrew/bin/brew || -e /usr/local/bin/brew ]]; then
		log_info "Installing Homebrew..."
		sudo -v # Prompt for sudo to activate cahced login. Not good to run the whole brew installation command as root, not intended as such.
...
step_mise() {
	is_workstation || {
		log_info "Non-workstation environment — skipping mise."
		return 0
	}
...
step_crontab() {
	is_workstation || {
		log_info "Non-workstation environment — skipping crontab."
		return 0
	}
...
step_ghq() {
	is_workstation || {
		log_info "Non-workstation environment — skipping ghq."
		return 0
	}
