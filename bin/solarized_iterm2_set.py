...
# What does work is to in the shebang above to use the python version installed by Iterm2.  I will also have the required package iterm2 installed.
# 1. iTerm> Scripts > Manage > Install python runtime
# 2. iTerm> Script > Manage > Open Python REPL > allow Python API usage.
# 3. Create symlink as there can't be space in shebang path above:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Set Solarized Color Preset in iterm2."
    )
    ...
    if args.mode not in ("dark", "light"):
        print(
            "mode argument must be in ('dark', 'light'). '{:s}' was given".format(
                args.mode
            ),
            file=sys.stderr,
        )
