...
#   - ugly mirc formatting patch by Juhamatti Niemelä
#
# Version history:
#  1.5: - Changed to handle properly the string length when mirc colors
#  	  and formatting is used in the trackbar_string variable.
...
sub line {
    ...
    # String length without the colors and formatting.
    # Regexp ripped from cleanpublic.pl script.
    my $tmp = $string;
    $tmp =~ s/\x03\d?\d?(,\d?\d?)?|\x02|\x1f|\x16|\x06|\x07//g;
    ...
    # Ugly hack which determines correct end point to the trackbar
    # line so that it won't wrap to the next line ignoring the mirc
    # color formatting.
    ...
    my $ctrl = 0;
    my @a = split(//, $string);
    while(($c < $width) && ($i < length($string))) {
    	if ($a[$i] =~ m/\x03/g) {
		$ctrl = 1;
	} elsif ($a[$i] =~ m/\x02|\x1f|\x16|\x06|\x07/g) {
		$ctrl = 0;
	} elsif (($a[$i] =~ m/[^\d]/g) && ($ctrl)) {
		$c++;
		$ctrl = 0;
	} elsif($ctrl == 0) {
		$c++;
    ...
    return $config{'trackbar_style'} . substr($string, 0, $i);
}
